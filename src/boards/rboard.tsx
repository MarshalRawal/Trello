...
export default function Boards({ allData, setAllData }) {
  ...
  return (
    Object.values(allData.boards).map((board)=>{
      return(
      ((board.id === allData.activeBoardId) &&   <div className="flex gap-5 pl-5 pt-15 min-h-screen w-full items-start overflow-x-auto overflow-y-hidden" key={board.id} style={{backgroundImage:`url(${board.image})`
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center relative">
                {isEditing !== list.id?
                <p className="leading-none w-full cursor-pointer" onClick={()=>setIsEditing(list.id)}>{list.title}</p>
                :<input autoFocus onBlur={()=>setIsEditing("")} value={list.title} onChange={(e)=>setAllData((adata)=>{return {...adata,lists:{...adata.lists,[list.id]:{...adata.lists[list.id],title:e.target.value}}}})} className="w-full"></input>}
                <button
                ...
                <div className="absolute top-10 right-[-11.5rem]  bg-amber-400 w-52  pt-2 pb-3 pr-2">
                  <div className="flex items-center justify-between">
                    ...
                    <p>List Actions</p>
                    <button className="cursor-pointer" onClick={()=>setListDetails((ld)=>{return {...ld,id:""}})}>
                        <svg
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                  ...
                  <div className="flex flex-col items-start pl-2">
                    ...
                    <button onClick={()=>{
                      ...
                      setListDetails({id:""});
                      // set
                    }} className="cursor-pointer">Edit List</button>
                    <button onClick={()=>{
                      
                    }}>Archieve List</button>
                  </div>
                </div>
              )}
              </div>
              ...
              {isActiveList === list.id && (
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setIsActiveList("")}>Cancel</button>
                  </div>
                </div>
              )}
            </div>
